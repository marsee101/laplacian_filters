...
// 2015/07/19 : for 文の中の if 文が少なくなるように変更した。左側、上側2ピクセルずつが0
...
int main()
{
    ...
    for (y=0, line_sel=0; y<VERTICAL_PIXEL_WIDTH; y++){
        switch(line_sel){ // first line, y=2 012, y=3 120, y=4 201
            case 2 :
                fl = 0; sl = 1; tl = 2;
                break;
            case 0 :
        ...
        memcpy((void *)line_buf[y%3], (const int*)cam_fb_addr, HORIZONTAL_PIXEL_WIDTH*sizeof(int));

        for (x=0; x<HORIZONTAL_PIXEL_WIDTH; x++)
            line_buf[y%3][x] = conv_rgb2y(line_buf[y%3][x]);    // カラーから白黒へ

        for (x=0; x<HORIZONTAL_PIXEL_WIDTH; x++){
            if ((y < 2) ||(x < 2)){ // 上2列、左横2行の境界の時の値は0とする
                lap_buf[x] = 0;
                continue;
            }
